...
def get_selinux_status(client: paramiko.SSHClient, username):
    try:
        ...
        if username == "root":
            stdin, stdout, stderr = client.exec_command("getenforce")
        else:
            stdin, stdout, stderr = client.exec_command("getenforce")
...
def get_ports_linux(client : paramiko.SSHClient):
    try:
        ...
        return None
def get_kube_pods(client: paramiko.SSHClient):
    try:
        logging.info("Fetching kubernetes pods details")
        # client.exec_command("echo '{0}'| sudo -S su".format(root))
        stdin, stdout, stderr = client.exec_command("kubectl get pods --all-namespaces | grep -v 'default'")
...
def get_kube_svc(client: paramiko.SSHClient):
    try:
        logging.info("Fetching kubernetes services")
        stdin, stdout, stderr = client.exec_command("kubectl get svc --all-namespaces | grep -v 'aiops'")
        # stdin, stdout, stderr = client.exec_command(
...
def get_kube_nodes(client: paramiko.SSHClient):
    try:
        logging.info("Fetching kubernetes nodes.")
        stdin, stdout, stderr = client.exec_command("kubectl get nodes | awk 'NR>1'")
        # stdin, stdout, stderr = client.exec_command(
...
def get_memory_usage_linux(client: paramiko.SSHClient):
    try:
        ...
        stdin, stdout, stderr = client.exec_command('free -m | awk "NR==2"')
        err = stderr.read().decode()
        if err:
            # print("Error ", err)
...
def fetch_uptime_windows(connection : paramiko.SSHClient):
    try:
        ...
        stdin, stdout, stderr = client.exec_command("net statistics workstation")
        err = stderr.read().decode()
        if err:
...
def fetch_service_status_windows(connection, service_name):
    try:
        ...
        stdin, stdout, stderr = client.exec_command("sc query {0}".format(service_name))
        err = stderr.read().decode()
        if err:
...
def get_last_boot_linux(client: paramiko.SSHClient):
    try:
...
def get_uptime_linux(client: paramiko.SSHClient):
    try:
...
def get_service_status_linux(client: paramiko.SSHClient, service_name):
    try:
        logging.info("Fetching status for service: "+service_name)
        if len(service_name) == 0:
            logging.warning("Empty service name.")
            return None
        stdin, stdout, stderr = client.exec_command("systemctl status {0}".format(service_name))
        err = stderr.read().decode()
        if err:
...
def get_cpu_usage_linux(client: paramiko.SSHClient):
    try:
        ...
        stdin, stdout, stderr = client.exec_command('mpstat -P ALL | awk "NR==4"')
        err = stderr.read().decode()
        if err:
...
def get_memory_usage_list_linux(client: paramiko.SSHClient):
    try:
        ...
        stdin, stdout, stderr = client.exec_command('ps -eo cmd,%mem --sort=-%mem | awk "NR>1 && NR<=6"')
        err = stderr.read().decode()
        if err:
            logging.error("Error while fetching memory usage contributors for linux: "+err)
            # print("Error ", err)
            return
        # print("Memory Usage------------->")
        result = stdout.read().decode().strip().split("\n")
...
def fetch_disk_usage_list_windows(connection):
    try:
        ...
        stdin, stdout, stderr = connection.exec_command('wmic logicaldisk get deviceid, size,freespace')
        err = stderr.read().decode()
        if err:
            # print("Error ", err)
...
def get_disk_usage_linux(client: paramiko.SSHClient):
    try:
        ...
        stdin, stdout, stderr = client.exec_command('df | awk "NR>1"')
        err = stderr.read().decode()
        if err:
            # print("Error ", err)
...
def get_cpu_usage_list_linux(client: paramiko.SSHClient):
    try:
        ...
        stdin, stdout, stderr = client.exec_command('ps -eo cmd,%cpu --sort=-%cpu | awk "NR>1 && NR<=6"')
        err = stderr.read().decode()
        if err:
            # print("Error ", err)
            logging.error(
...
def fetch_os_version_windows(connection):
    try:
        ...
        stdin, stdout, stderr = connection.exec_command('systeminfo')
        err = stderr.read().decode()
        if err:
            # print("Error ", err)
            logging.error(
...
if __name__ == "__main__":
    ...
    if not os.path.exists(os.path.join(home_path,"braio_dhc_reports_html")):
        os.makedirs(os.path.join(home_path,"braio_dhc_reports_html"))
    if not os.path.exists(os.path.join(home_path,'braio_dhc_reports_csv')):
        os.makedirs(os.path.join(home_path,'braio_dhc_reports_csv'))
    file_name_format = datetime.strftime(datetime.today(), "%d%m%Y%H%M%S")
    file_name = os.path.join(home_path,'braio_dhc_reports_html/dhc_report_{0}.html').format(
        file_name_format)
    csv_folder =   os.path.join(home_path,'braio_dhc_reports_csv')
            ...
            try:
                ...
                if server['os_type'].lower() == "windows":
                    ...
                    if connection is None:
                        ...
                        os_version, total_memory, total_mem_usage = fetch_os_version_windows(connection)
                    ...
                    if client is None:
                        ...
                        days_up = get_uptime_linux(client)
                        boot_time = get_last_boot_linux(client)
                        ...
                        for service in server['service']:
                            if len(service) == 0:
                                continue
                            service_status = get_service_status_linux(client, service)
                            color = 'red'
                            if service_status == None:
                                service_status = "Service Not Found"
                            elif service_status.lower() == 'running':
                        ...
                        service_status = get_selinux_status(client,server['username'])
                        ...
                        cpu_usage_list[index] = get_cpu_usage_list_linux(client)

                        mem_usage_list[index] = get_memory_usage_list_linux(client)
                        other_used = 0
                        other_total = 0
                        if disk_usage_list is None:
                        ...
                        kube_nodes_list[index] = get_kube_nodes(client)
                        kube_pods_list[index] = get_kube_pods(client)
                        kube_service_list[index] = get_kube_svc(client)
                        
                        if 'ports' in server.keys():
                            result = get_ports_linux(client)
