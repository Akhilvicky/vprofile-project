...
def check_url(file_name, urls: list):
    ...
    is_exist = os.path.isfile(csv_folder+ '/url_status.csv')
    url_status_csv = open(csv_folder+'/url_status.csv', 'a', encoding='UTF-8', newline='')
    url_writer = csv.writer(url_status_csv)
    if not is_exist:
            url_writer.writerow(['URL', 'Status','Rag Status','Timestamp'])
        ...
        for url in urls:
            try:
                ...
                if status_code != 404 and status_code < 500:
                    ...
                    url_writer.writerow([url,"Active",1,current_time])
                    ...
                    url_writer.writerow([url,"Inactive",0,current_time])
            except HTTPError:
                content += '''<tr>
                                <td>{0}</td>
                                <td>Active</td>
...
def get_ports_linux(client : paramiko.SSHClient, root):
    try:
        ...
        stdin, stdout, stderr = client.exec_command("netstat -tulpn | grep LISTEN", get_pty=True)
        err = stderr.read().decode()
        if err:
...
def update_server_status(file_name, csv_folder, tool):
    try:
        current_time = datetime.strftime(datetime.now(), "%Y-%m-%d %H:%M:%S")
        current_time = datetime.strptime(current_time, "%Y-%m-%d %H:%M:%S")
        is_exist = os.path.isfile(csv_folder+'/clarios_preprod_braio_server_status.csv')
        
        csv_file = open(csv_folder+'/clarios_preprod_braio_server_status.csv', 'a', encoding='UTF-8', newline='')
...
def kubernetes_certificate_Expiry_Check(filename):
    current_time = datetime.strftime(datetime.now(), "%Y-%m-%d %H:%M:%S")
    current_time = datetime.strptime(current_time, "%Y-%m-%d %H:%M:%S")
    is_exist = os.path.isfile(csv_folder+ '/Kube_Certs_status.csv')
    Kube_Certs_status_csv = open(csv_folder+'/Kube_Certs_status.csv', 'a', encoding='UTF-8', newline='')
    Kube_Certs_writer = csv.writer(Kube_Certs_status_csv)
    if not is_exist:
            Kube_Certs_writer.writerow(['Rag Status', 'Certificate', 'Expiration Date', 'Days Remaining','FOR','Timestamp'])
    logging.info("Starting Kubernetes Certification Expiry Check")
    content= ''''''
    try:
            cmd_kube_cert = ["kubeadm","certs","check-expiration"]
            output_kube_cert = subprocess.check_output(cmd_kube_cert, universal_newlines=True)
            r=output_kube_cert.split("\n")
            ...
            for j in range(len(r)):
                cert_name.append(r[j][0])
                Expiry_date.append(r[j][1]+" "+r[j][2]+" "+r[j][3]+" "+r[j][4]+" "+r[j][5])
                Days_Remaining.append(r[j][6])
                        ...
                        <thead class="thead-dark">
                            <tr>
                                <th scope="col" colspan="4" style="text-align: center;">
                                    <h4>KUBERNETES CERTIFICATE EXPIRATION STATUS</h4>
                                </th>
                            </tr>
                        </thead>
                        <thead class="thead-light">
            ...
            for p in range(len(cert_name)): 
                ...
                content+= '''
                            ...
                            '''.format(cert_name[p],Expiry_date[p],Days_Remaining[p])
                print(cert_name[p],Expiry_date[p],Days_Remaining[p])
                Kube_Certs_writer.writerow(
                        [status_dict[color],cert_name[p],Expiry_date[p],Days_Remaining[p],"Kubernetes",current_time])
            ...
            logging.info("Kubernetes Certification Expiry Check Completed")
            append_to_file(file_name, content)
            Kube_Certs_status_csv.close()
    except:
        err_type, value, traceback = sys.exc_info()
        # print('{0} at line {1} in {2}'.format(str(value), str(
        #     traceback.tb_lineno), str(traceback.tb_frame.f_code.co_filename)))
        logging.error("Exception while checking the Kubernetes Certification Expiry Check: "+str(value))
        return None
    
def ssl_certificate(file_name, CERT_PATHS_L: list):   #Set the paths to the certificate
    import csv
    current_time = datetime.strftime(datetime.now(), "%Y-%m-%d %H:%M:%S")
    current_time = datetime.strptime(current_time, "%Y-%m-%d %H:%M:%S")
    is_exist = os.path.isfile(csv_folder+ '/ssl_status.csv')
    ssl_status_csv = open(csv_folder+'/ssl_status.csv', 'a', encoding='UTF-8', newline='')
    ssl_writer = csv.writer(ssl_status_csv)
    if not is_exist:
            ssl_writer.writerow(['Rag Status', 'Certificate', 'Expiration Date', 'Days Remaining','FOR','Timestamp'])
    
    kubernetes_certificate_Expiry_Check(file_name)
    ...
    try:
        ...
        for CERT in CERT_PATHS_L:
            cmd = ["openssl", "x509", "-enddate", "-noout", "-in", CERT]
            output = subprocess.check_output(cmd, universal_newlines=True) 
        # Extract the expiration date
            exp_date = output.split("=")[1].strip()
           
        # Convert the expiration date to epoch time
            exp_epoch = int(subprocess.check_output(
            ["date", "+%s", "-d", exp_date]))

        # Get the current epoch time

            now_epoch = int(subprocess.check_output(["date", "+%s"]))

        # Calculate the number of days until the certificate expires
            days_remaining = (exp_epoch - now_epoch) // (60*60*24)

        # Add the certificate details to the HTML report
             
            color = 'red'
            if days_remaining >=30 and days_remaining < 60:
                color = 'amber'
            ...
            content+= '''
                    <tr>
                        <th><div class="{0}-circle"></div></th>
                        <td>{1}</td>
                        <td>{2}</td>
                        <td>{3}</td>
                    </tr>
                    '''.format(color, CERT,exp_date,days_remaining)
            ssl_writer.writerow(
                [status_dict[color], CERT,exp_date,days_remaining,"SSL",current_time])
        ...
        append_to_file(file_name, content)
        logging.info("SSL EXPIRATION STATUS COMPLETED")
        ssl_status_csv.close()
    except:
        err_type, value, traceback = sys.exc_info()
        # print('{0} at line {1} in {2}'.format(str(value), str(
        #     traceback.tb_lineno), str(traceback.tb_frame.f_code.co_filename)))
        logging.error("Exception while checking the ssl expiration status: "+str(value))
        return None
...
def generate_table(file_name, csv_folder):
    try:
        ...
        current_time = datetime.strptime(current_time, "%Y-%m-%d %H:%M:%S")
        content = ''''''
        is_exist = os.path.isfile(csv_folder+'/clarios_preprod_braio_service_status.csv')
        service_csv = open(csv_folder+'/clarios_preprod_braio_service_status.csv', 'a', encoding='UTF-8', newline='')
        service_writer = csv.writer(service_csv)
        if not is_exist:
            service_writer.writerow(['Hostname', 'Status', 'Service Name', 'State', 'Timestamp'])
        
        is_exist = os.path.isfile(csv_folder+'/clarios_preprod_braio_cpu_utilization.csv')
        cpu_utilization_csv = open(csv_folder+'/clarios_preprod_braio_cpu_utilization.csv', 'a', encoding='UTF-8', newline='')
        cpu_writer = csv.writer(cpu_utilization_csv)
        if not is_exist:
            cpu_writer.writerow(['Hostname', 'RAG Status', 'Process Name', '% Utilization','Timestamp'])
        
        is_exist = os.path.isfile(csv_folder+'/clarios_preprod_braio_memory_utilization.csv')
        memory_utilization_csv = open(csv_folder+'/clarios_preprod_braio_memory_utilization.csv', 'a', encoding='UTF-8', newline='')
        ...
        is_exist = os.path.isfile(csv_folder+'/clarios_preprod_braio_disk_utilization.csv')
        disk_utilization_csv = open(csv_folder+'/clarios_preprod_braio_disk_utilization.csv', 'a', encoding='UTF-8', newline='')
        ...
        is_exist = os.path.isfile(csv_folder+'/clarios_preprod_braio_ports_status.csv')
        ports_csv = open(csv_folder+'/clarios_preprod_braio_ports_status.csv', 'a', encoding='UTF-8', newline='')
        port_writer = csv.writer(ports_csv)
        if not is_exist:
            port_writer.writerow(['Hostname', 'RAG Status', 'Port Number', 'Status','Timestamp'])
        
        is_exist = os.path.isfile(csv_folder+'/clarios_preprod_braio_kube_pods.csv')
        kube_pods_csv = open(csv_folder+'/clarios_preprod_braio_kube_pods.csv', 'a', encoding='UTF-8', newline='')
        pods_writer = csv.writer(kube_pods_csv)
        if not is_exist:
            pods_writer.writerow(['Hostname', 'RAG Status', 'Namespace', 'Name', 'Status', 'Age','Timestamp'])
        
        
        is_exist = os.path.isfile(csv_folder+'/clarios_preprod_braio_kube_svc.csv')
        kube_svc_csv = open(csv_folder+'/clarios_preprod_braio_kube_svc.csv', 'a', encoding='UTF-8', newline='')
        svc_writer = csv.writer(kube_svc_csv)
        if not is_exist:
            svc_writer.writerow(['Hostname', 'Namespace', 'Name', 'Cluster-IP', 'Port', 'Age','Timestamp'])
        
        is_exist = os.path.isfile(csv_folder+'/clarios_preprod_braio_kube_nods.csv')
        kube_nods_csv = open(csv_folder+'/clarios_preprod_braio_kube_nods.csv', 'a', encoding='UTF-8', newline='')
        nods_writer = csv.writer(kube_nods_csv)
        if not is_exist:
            nods_writer.writerow(['Hostname', 'RAG Status', 'Name', 'Status', 'Age','Timestamp'])
        
        is_exist = os.path.isfile(csv_folder+'/clarios_preprod_braio_kafka_groups.csv')
        kafka_group_csv = open(csv_folder+'/clarios_preprod_braio_kafka_groups.csv', 'a', encoding='UTF-8', newline='')
        group_writer = csv.writer(kafka_group_csv)
        if not is_exist:
            group_writer.writerow(['Hostname', 'Group','Timestamp'])
        
        is_exist = os.path.isfile(csv_folder+'/clarios_preprod_braio_kafka_topics.csv')
        kafka_topic_csv = open(csv_folder+'/clarios_preprod_braio_kafka_topics.csv', 'a', encoding='UTF-8', newline='')
        topic_writer = csv.writer(kafka_topic_csv)
        if not is_exist:
            topic_writer.writerow(['Hostname', 'Topic','Timestamp'])
        
        is_exist = os.path.isfile(csv_folder+'/clarios_preprod_braio_kafka_lags.csv')
        kafka_lag_csv = open(csv_folder+'/clarios_preprod_braio_kafka_lags.csv', 'a', encoding='UTF-8', newline='')
...
def unset_proxy(client):
    try:
        logging.info("Unsetting Proxy")
        stdin, stdout, stderr = client.exec_command(
        ...
        if err:
            # print("Error ", err)
            logging.error(
                "Error error while unsetting proxy "+err)
            return
        # print("CPU Usage------------->")
        result = stdout.read().decode().strip().split("\n")
        # print(response)
        logging.info("Successfully unset proxy")
        return result
    except:
        err_type, value, traceback = sys.exc_info()
        # print('{0} at line {1} in {2}'.format(str(value), str(
        #     traceback.tb_lineno), str(traceback.tb_frame.f_code.co_filename)))
        logging.error(
            "Error while unsetting proxy "+str(value))
        return None
...
def send_mail(filename, csv_folder):
    ...
    try: 
        msg =MIMEMultipart()
        msg['Subject']='Clarios PRE-PROD DHC Report'
        ...
        cc = ["NAUT-IP-Support@lntinfotech.com", "ravi.deo@lntinfotech.com"]
        to = [""]+cc
        print(cc)
...
if __name__ == "__main__":
    home_path = '/home/xbhagatdSA/DHC_Scripts_and_modules'
    ...
    if "urls" in input_data:
        try:
            ...
            print(str(ex))
    if "CERT_PATHS" in input_data:
        try:
            CERT_PATHS = input_data['CERT_PATHS']
            ssl_certificate(file_name, CERT_PATHS)
        except Exception as ex:
            print(str(ex))
